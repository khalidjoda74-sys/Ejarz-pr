...
const anonymizeOwnedCouncils = async (uid: string): Promise<string[]> => {
  const ids: string[] = [];
  while (true) {
    ...
    await writer.close();
    if (snapshot.size < 300) return ids;
  }
};

const anonymizeComments = async (uid: string): Promise<void> => {
  while (true) {
...
const anonymizeConversations = async (uid: string): Promise<string[]> => {
  const ids: string[] = [];
  ...
  for (const doc of snapshot.docs) {
    ...
    if (data.lastSenderId === uid) update.lastSenderId = null;
...
const anonymizeMessages = async (uid: string): Promise<void> => {
  while (true) {
...
const anonymizeVotes = async (uid: string): Promise<void> => {
  while (true) {
...
const deleteBlocksTargetingUser = async (uid: string): Promise<void> => {
  while (true) {
...
const anonymizeReports = async (uid: string): Promise<void> => {
  while (true) {
...
const anonymizeSponsorshipRequests = async (uid: string): Promise<void> => {
  while (true) {
...
export const deleteMyAccount = onCall(
  ...
  async (request) => {
    ...
    ]).then((results) => [results[0] as string[], results[1] as string[]]);
