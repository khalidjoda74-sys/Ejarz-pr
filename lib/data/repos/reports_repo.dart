...
class ReportsFilters {
  ...
  static DateTime _atEndOfDay(DateTime d) =>
...
class ReportsData {
  ...
  int get maintenanceTotal =>
      maintenanceNew + maintenanceInProgress + maintenanceDone;
  ...
  ReportsData copyWith({
    ...
    return ReportsData(
      ...
      propertyUnitsOccupied:
          propertyUnitsOccupied ?? this.propertyUnitsOccupied,
      ...
      invoicesFromContracts:
          invoicesFromContracts ?? this.invoicesFromContracts,
      invoicesFromMaintenance:
          invoicesFromMaintenance ?? this.invoicesFromMaintenance,
...
class ReportsRepo {
  ...
  ReportsData _compute(ReportsFilters f) {
    // فلترة المؤرشف
    final tenants = f.includeArchived
        ? _tenants
        : _tenants.where((m) => !(_tryBool(m['isArchived']) ?? false)).toList();
    final properties = f.includeArchived
        ? _properties
        : _properties
            .where((m) => !(_tryBool(m['isArchived']) ?? false))
            .toList();
    final contracts = f.includeArchived
        ? _contracts
        : _contracts
            .where((m) => !(_tryBool(m['isArchived']) ?? false))
            .toList();
    final invoices = f.includeArchived
        ? _invoices
        : _invoices
            .where((m) => !(_tryBool(m['isArchived']) ?? false))
            .toList();
    final maintenance = f.includeArchived
        ? _maintenance
        : _maintenance
            .where((m) => !(_tryBool(m['isArchived']) ?? false))
            .toList();
    ...
    for (final p in properties) {
      ...
      final isBuilding = _containsAny(
          (_tryString(p['type']) ?? '').toLowerCase(),
          const ['building', 'عمارة']);
      final rentalMode = (_tryString(p['rentalMode']) ?? '').toLowerCase();
      final perUnit = isBuilding &&
          (rentalMode.contains('unit') ||
              rentalMode.contains('perunit') ||
              rentalMode.contains('وحد'));
    ...
    for (final c in contracts) {
      ...
      final start = _tryDate(c['startDate']) ??
          _tryDate(c['startsOn']) ??
          _tryDate(c['startAt']);
      final end = _tryDate(c['endDate']) ??
          _tryDate(c['endsOn']) ??
          _tryDate(c['endAt']);
    ...
    for (final inv in invoices) {
      ...
      final amount = (_tryNum(inv['amount']) ??
              _tryNum(inv['total']) ??
              _tryNum(inv['net']) ??
              _tryNum(inv['grandTotal']) ??
              0)
          .toDouble();

      final paidAt = _tryDate(inv['paidAt']) ??
          _tryDate(inv['paid_on']) ??
      ...
      final isPaid = !isCanceled &&
          (paidAt != null || paidAmount >= (amount.abs() - 0.000001));
      final paidAnchor = paidAt ?? issueDate ?? dueOn ?? todayEnd;
      final remaining = (amount.abs() - paidAmount).clamp(0.0, double.infinity);

      final typeStr =
          (_tryString(inv['type']) ?? _tryString(inv['category']) ?? '')
              .toLowerCase();
      final noteStr =
          (_tryString(inv['note']) ?? _tryString(inv['description']) ?? '')
              .toLowerCase();
      final isExpense = (amount < 0) ||
          typeStr.contains('expense') ||
          typeStr.contains('مصروف') ||
          typeStr.contains('cost') ||
          typeStr.contains('out') ||
          typeStr.contains('صرف') ||
          noteStr.contains('مصروف');

      if (isPaid && !isExpense) {
        if (!f.hasDate || f.inRange(paidAnchor)) rev += amount.abs();
      }
      if (isPaid && isExpense) {
        if (!f.hasDate || f.inRange(paidAnchor)) exp += amount.abs();
      }
  ...
  static bool _contractIsActive(
      DateTime? start, DateTime? end, bool terminated, DateTime today) {
  ...
  static bool _isMaintInvoiceAny(Map<String, dynamic> inv) {
    final linkedMaintenance = inv['maintenanceRequestId'] ??
        inv['maintenanceId'] ??
        inv['maintenance_id'] ??
        inv['maintenance'] ??
        inv['maintenanceSnapshot'];
    if (linkedMaintenance is String && linkedMaintenance.trim().isNotEmpty) {
      return true;
    }
    if (linkedMaintenance is Map && linkedMaintenance.isNotEmpty) return true;
    final note = (_tryString(inv['note']) ??
            _tryString(inv['notes']) ??
            _tryString(inv['description']) ??
            '')
        .toLowerCase();
    final type = (_tryString(inv['type']) ??
            _tryString(inv['category']) ??
            _tryString(inv['sourceType']) ??
            '')
        .toLowerCase();
    return note.contains('maintenance') ||
  ...
  static bool _isContractInvoiceAny(Map<String, dynamic> inv) {
    ...
    if (cid is Map) {
      final inner =
          cid['id'] ?? cid['contractId'] ?? cid['ref'] ?? cid['refId'];
  ...
  static bool? _tryBool(dynamic v) {
    ...
    if (v is String) {
      return ['1', 'true', 'yes', 'y', 'on'].contains(v.toLowerCase());
    }
    return null;
  }
