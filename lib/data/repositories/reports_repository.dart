...
class ReportsRepository {
  ...
  bool inRange(DateTime? d) {
    ...
    if (to != null &&
        d.isAfter(DateTime(to!.year, to!.month, to!.day, 23, 59, 59, 999))) {
      return false;
    }
    return true;
  }
  ...
  ReportsSummary getSummary() {
    ...
    final invBox =
        HiveService.boxIfOpen(kInvoicesBox); // ← بعد استيرادها بالأعلى
    ...
    for (final p in props) {
      ...
      final occUnits =
          _asInt(_read(p, const ['occupiedUnits'])) ?? (isOcc ? units : 0);
    ...
    for (final t in tenants) {
      final hasActive =
          (_asBool(_read(t, const ['hasActiveContract'])) ?? false) ||
              ...
              (_read(t, const ['contractId', 'activeContractId', 'contract']) !=
                  null);
      if (hasActive) {
        bound++;
      } else {
    ...
    for (final inv in invoices) {
      ...
      if (!isPaid) {
        if (!hasDate) {
          ...
          final anchor = dueOn ?? KsaTime.now();
  ...
  static bool _contractIsActive(
      DateTime? start, DateTime? end, bool terminated) {
    ...
    try {
      switch (name) {
        ...
        case 'activeContractId':
          return d.activeContractId;
        case 'contractsCount':
          return d.contractsCount;
        case 'startDate':
          return d.startDate;
        case 'endDate':
          return d.endDate;
  ...
  static bool? _asBool(dynamic v) {
    ...
    if (v is String) {
      return ['1', 'true', 'yes', 'y', 'نعم'].contains(v.toLowerCase());
    }
    return null;
  }
}
