...
const List<String> kContractsAliases = [
  kContractsBox,
  'contracts',
  'contracts_box'
];
const List<String> kMaintenanceAliases = [
  kMaintenanceBox,
  'maintenance',
  'maintenance_box'
];
...
class HiveService {
  ...
  static Future<String?> _ensureOneOfOpenTypedFirst<T>(
  ...
  static Future<void> _migrateMaintenanceIfNeeded({
    ...
    try {
      ...
      final Box src = Hive.box(fromName); // قد يحتوي عناصر غير متوافقة
      final Box<MaintenanceRequest> dst =
          Hive.box<MaintenanceRequest>(toName); // الهدف Typed
  ...
  static Future<void> ensureReportsBoxesOpen() async {
    ...
    _contractsBoxNameInUse ??= await _ensureOneOfOpenTypedFirst<Contract>(
        _aliasedNamesForUser(kContractsAliases));
    ...
    _maintenanceBoxNameInUse ??=
        await _ensureOneOfOpenTypedFirst<MaintenanceRequest>(
            _aliasedNamesForUser(kMaintenanceAliases));
  ...
  static Listenable mergedReportsListenable() {
    final Listenable t = (typedBoxIfOpen<Tenant>(boxName(kTenantsBox))
            ?.listenable() as Listenable?) ??
        ValueNotifier<bool>(false);
    final Listenable p = (typedBoxIfOpen<Property>(boxName(kPropertiesBox))
            ?.listenable() as Listenable?) ??
        ValueNotifier<bool>(false);
    final Listenable i = (typedBoxIfOpen<Invoice>(boxName(kInvoicesBox))
            ?.listenable() as Listenable?) ??
        ValueNotifier<bool>(false);
    final Listenable s =
        (boxIfOpen(boxName(kSessionBox))?.listenable() as Listenable?) ??
            ValueNotifier<bool>(false);
  ...
  static String contractsBoxName() =>
      _contractsBoxNameInUse ?? boxName(kContractsBox);

  /// API مساعد: اسم صندوق الصيانة الفعلي الذي تم فتحه (خاص بالمستخدم)
  static String maintenanceBoxName() =>
      _maintenanceBoxNameInUse ?? boxName(kMaintenanceBox);
}
