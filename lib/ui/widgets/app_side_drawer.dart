...
import 'package:flutter/services.dart'; // MaxLengthEnforcement
...
if (endInclusiveKsaDateOnly == null && endAtUtc != null) {
  ...
  // ==== الإعدادات: قراءة / حفظ Firestore — على user_prefs/{uid} ====
   Future<_SettingsData> _fetchSettings() async {
    ...
    try {
      ...
      // ⬅️ بدون إنترنت أو خطأ في Firestore → نرجع القيم الافتراضية
      // معتمدة على _SettingsData.empty (7 / 15 / 30 / 45 ...)
      return _SettingsData.empty();
    }

    String _read(String k, String fallback) => (m[k] ?? fallback).toString();

    final result = _SettingsData(
      language: _read('settings_language', 'ar'),
      dateSystem: _read('settings_date_system', 'gregorian'),
      ...
      annualDays: _toInt(m['notif_annual_days'], 45),

      // رسائل الدفعات قبل الانتهاء
      monthlyMsgBefore: _read(
        'notif_monthly_msg_before',
        _read('notif_monthly_msg', _kDefaultMsg),
      ),
      quarterlyMsgBefore: _read(
        'notif_quarterly_msg_before',
        _read('notif_quarterly_msg', _kDefaultMsg),
      ),
      semiAnnualMsgBefore: _read(
        'notif_semiannual_msg_before',
        _read('notif_semiannual_msg', _kDefaultMsg),
      ),
      annualMsgBefore: _read(
        'notif_annual_msg_before',
        _read('notif_annual_msg', _kDefaultMsg),
      ),

      // رسائل عند الانتهاء
      monthlyMsgOn: _read('notif_monthly_msg_on', _kDefaultMsgEnded),
      quarterlyMsgOn: _read('notif_quarterly_msg_on', _kDefaultMsgEnded),
      semiAnnualMsgOn: _read('notif_semiannual_msg_on', _kDefaultMsgEnded),
      annualMsgOn: _read('notif_annual_msg_on', _kDefaultMsgEnded),
      ...
      contractAnnualDays: _toInt(
        m['notif_contract_annual_days'],
        _toInt(m['notif_annual_days'], 45),
      ),

      // تبويب عقود: الرسائل قبل الانتهاء
      contractMonthlyMsgBefore: _read(
        'notif_contract_monthly_msg_before',
        _read('notif_monthly_msg_before', _kDefaultMsg),
      ),
      contractQuarterlyMsgBefore: _read(
        'notif_contract_quarterly_msg_before',
        _read('notif_quarterly_msg_before', _kDefaultMsg),
      ),
      contractSemiAnnualMsgBefore: _read(
        'notif_contract_semiannual_msg_before',
        _read('notif_semiannual_msg_before', _kDefaultMsg),
      ),
      contractAnnualMsgBefore: _read(
        'notif_contract_annual_msg_before',
        _read('notif_annual_msg_before', _kDefaultMsg),
      ),
    );
  ...
  final md = _asInt(payload['notif_monthly_days']);
  final qd = _asInt(payload['notif_quarterly_days']);
  final sd = _asInt(payload['notif_semiannual_days']);
  final ad = _asInt(payload['notif_annual_days']);
  ...
  // ✅ يضمن وجود مستند users/{uid} لدعم القواعد التي تشترط وجوده
    Future<void> _ensureUsersDocForRules(
      FirebaseFirestore fs, String uid) async {
    try {
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (_) => Directionality(
        ...
        child: AlertDialog(
          ...
          title: Text('إعادة ضبط كامل', style: GoogleFonts.tajawal(fontWeight: FontWeight.w900)),
          content: Text(
            'سيتم حذف جميع بياناتك (العقارات، العقود، الفواتير، المستأجرون، الإشعارات، والإعدادات). هل تريد المتابعة؟',
            style: GoogleFonts.tajawal(fontSize: 14.sp, height: 1.6, fontWeight: FontWeight.w700),
          ),
          ...
          actions: [
            ElevatedButton(
              ...
              child: Text('تأكيد الحذف', style: GoogleFonts.tajawal(fontWeight: FontWeight.w900)),
            ),
            TextButton(
...
Future<void> _factoryReset(BuildContext context) async {
  ...
  Future<void> _deleteOwnedDocs(String collection, {required String field}) async {
  ...
  Future<void> _deleteOwnedDocsByFields(String collection, List<String> fields) async {
    for (final f in fields) {
      await _deleteOwnedDocs(collection, field: f);
    }
  }

  Future<void> _deleteUserNotifications() async {
    final notifCol = fs.collection('users').doc(uid).collection('notifications');
...
try {
  await Future.any([
    () async {
      ...
      await _deleteOwnedDocsByFields('properties', ownerFields);
      await _deleteOwnedDocsByFields('contracts', ownerFields);
      await _deleteOwnedDocsByFields('invoices', ownerFields);
      await _deleteOwnedDocsByFields('tenants', ownerFields);

      // مجموعات المستخدم الحالية تحت users/{uid}/...
      final uc = UserCollections(uid, db: fs);
      await _deleteUserSubcollection(uc.properties);
      await _deleteUserSubcollection(uc.contracts);
      await _deleteUserSubcollection(uc.invoices);
      await _deleteUserSubcollection(uc.tenants);
      await _deleteUserSubcollection(uc.maintenance);
      await _deleteUserSubcollection(uc.session);
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('تم حذف جميع بياناتك وإعادة ضبط التطبيق بنجاح.')),
    );
  } finally {
          ...
          child: FutureBuilder<_SubscriptionData>(
            ...
            builder: (context, snap) {
              ...
              final DateTime? startKsaDateOnly =
                  ...
                  (sub.startAtUtc != null ? KsaTime.dateOnly(KsaTime.toKsa(sub.startAtUtc!)) : null);
  ...
  Future<void> _openSettings(BuildContext context) async {
    final maxMsgLen = _kDefaultMsg.length;
    ...
    TextEditingController? cMsgMonthlyBeforeCtrl, cMsgQuarterlyBeforeCtrl, cMsgSemiBeforeCtrl, cMsgAnnualBeforeCtrl;
...
padding: EdgeInsets.fromLTRB(
  16.w, 12.h, 16.w, 8.h + MediaQuery.of(sheetCtx).viewInsets.bottom,
),
...
// removed: old init from persistent variable
                      ...
                      int cAnnualDays = data.contractAnnualDays;

                      bool cMonthlyOpen = false, cQuarterlyOpen = false, cSemiOpen = false, cAnnualOpen = false;

                      
                      bool cDailyOpen = false;
// أي عقد مفتوح؟
                      bool monthlyOpen = false, quarterlyOpen = false, semiOpen = false, annualOpen = false;

                      
                      bool dailyOpen = false;
// أي تبويب مُختار لكل عقد؟ 0=قبل الانتهاء، 1=عند الانتهاء
                      int monthlyMode = 0, quarterlyMode = 0, semiMode = 0, annualMode = 0;

                      // تهيئة الكنترولرز
                      msgMonthlyBeforeCtrl ??= TextEditingController(text: data.monthlyMsgBefore);
                      msgMonthlyOnCtrl ??= TextEditingController(text: data.monthlyMsgOn);

                      msgQuarterlyBeforeCtrl ??= TextEditingController(text: data.quarterlyMsgBefore);
                      msgQuarterlyOnCtrl ??= TextEditingController(text: data.quarterlyMsgOn);

                      msgSemiBeforeCtrl ??= TextEditingController(text: data.semiAnnualMsgBefore);
                      msgSemiOnCtrl ??= TextEditingController(text: data.semiAnnualMsgOn);

                      msgAnnualBeforeCtrl ??= TextEditingController(text: data.annualMsgBefore);
                      msgAnnualOnCtrl ??= TextEditingController(text: data.annualMsgOn);

                      // Controllers الخاصة بتبويب "العقود" (قبل الانتهاء فقط)
                      cMsgMonthlyBeforeCtrl ??= TextEditingController(text: data.contractMonthlyMsgBefore);
                      cMsgQuarterlyBeforeCtrl ??= TextEditingController(text: data.contractQuarterlyMsgBefore);
                      cMsgSemiBeforeCtrl ??= TextEditingController(text: data.contractSemiAnnualMsgBefore);
                      cMsgAnnualBeforeCtrl ??= TextEditingController(text: data.contractAnnualMsgBefore);
                      ...
                      return StatefulBuilder(
                        builder: (context, setState) {
                            ...
                            return _CardSection(
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Row(
                                    children: [
                                      Text('اللغة', style: GoogleFonts.tajawal(fontWeight: FontWeight.w800)),
                                      const Spacer(),
                                      Container(
                                        padding: EdgeInsets.symmetric(horizontal: 10.w, vertical: 6.h),
                                        decoration: BoxDecoration(
                                        ...
                                        child: Text(
                                          'العربية فقط',
                                          style: GoogleFonts.tajawal(fontWeight: FontWeight.w900, color: const Color(0xFF059669)),
                                        ),
                                      ),
                                    ],
                                  ),
                                  ...
                                  // نظام التاريخ
                                  Text('نظام التاريخ', style: GoogleFonts.tajawal(fontWeight: FontWeight.w800)),
                                  SizedBox(height: 8.h),
                                  Wrap(
                                    spacing: 8.w,
                                    children: [
                                      ChoiceChip(
                                        label: Text('ميلادي', style: GoogleFonts.tajawal(fontWeight: FontWeight.w900)),
                                        selected: dateSystem == 'gregorian',
                                        onSelected: (_) => setState(() => dateSystem = 'gregorian'), // ✅ تبديل فوري
                                      ),
                                      ChoiceChip(
                                        label: Text('هجري', style: GoogleFonts.tajawal(fontWeight: FontWeight.w900)),
                                        selected: dateSystem == 'hijri',
                                        onSelected: (_) => setState(() => dateSystem = 'hijri'), // ✅ تبديل فوري
                                      ),
                                    ],
                                  ),
                                  SizedBox(height: 12.h),
                          ...
                          Widget contractCard({
                            ...
                            final TextEditingController activeCtrl = (mode == 0) ? beforeCtrl : onCtrl;
                              ...
                              child: Column(
                                ...
                                children: [
                                  InkWell(
                                    ...
                                    child: Row(
                                      children: [
                                        Text(title, style: GoogleFonts.tajawal(fontWeight: FontWeight.w900, fontSize: 15.5.sp)),
                                        const Spacer(),
                                  ...
                                  if (expanded) ...[
                                    ...
                                    Text(
                                      ...
                                      style: GoogleFonts.tajawal(fontSize: 13.5.sp, fontWeight: FontWeight.w700, color: Colors.black.withOpacity(0.75)),
                                    ),
                                    SizedBox(height: 10.h),
                                    ...
                                    Row(
                                      children: [
                                        Text('أيام التنبيه مسبقًا', style: GoogleFonts.tajawal(fontWeight: FontWeight.w800)),
                                        const Spacer(),
                                        Container(
                                          padding: EdgeInsets.symmetric(horizontal: 10.w, vertical: 6.h),
                                          decoration: BoxDecoration(
                                          ...
                                          child: Text('$valueDays يوم', style: GoogleFonts.tajawal(fontWeight: FontWeight.w900, color: _primary)),
                                        ),
                                      ],
                                    ),
                                    Slider.adaptive(
                                ...
                                children: [
                                  InkWell(
                                    ...
                                    child: Row(
                                      children: [
                                        Expanded(
                                          child: Text(title, style: GoogleFonts.tajawal(fontWeight: FontWeight.w900, fontSize: 15.5.sp)),
                                        ),
...
Row(
  ...
  children: [
    ...
    Container(
      ...
      child: Text(
        '$valueDays يوم',
        style: GoogleFonts.tajawal(fontWeight: FontWeight.w900, color: _primary),
      ),
    ),
  ],
),
                                    ...
                                    children: [
                                      ...
                                      Row(
                                        children: [
                                          Expanded(
                                            child: ChoiceChip(
                                              label: Text('دورة السداد', style: GoogleFonts.tajawal(fontWeight: FontWeight.w900)),
                                              selected: topTab == 0,
                                              onSelected: (_) => setState(() { topTab = 0; }),
                                            ),
                                          ),
                                          SizedBox(width: 8.w),
                                          Expanded(
                                            child: ChoiceChip(
                                              label: Text('العقود', style: GoogleFonts.tajawal(fontWeight: FontWeight.w900)),
                                              selected: topTab == 1,
                                              onSelected: (_) => setState(() { topTab = 1; }),
                                            ),
                                          ),
                                        ],
                                      ),
                                          ...
                                          child: Column(
                                            ...
                                            children: [
                                              InkWell(
                                                onTap: () => setState(() {
  dailyOpen = !dailyOpen;
  if (dailyOpen) { monthlyOpen = quarterlyOpen = semiOpen = annualOpen = false; }
}),

                                                borderRadius: BorderRadius.circular(12.r),
                                                child: Row(
                                                  children: [
                                                    Expanded(child: Text('دفعات يومية (يوم)', style: GoogleFonts.tajawal(fontWeight: FontWeight.w900, fontSize: 15.5.sp))),
                                                    const Icon(Icons.expand_more_rounded, color: _primary),
                                                  ],
                                                ),
                                              ),
                                              AnimatedCrossFade(
                                                crossFadeState: dailyOpen ? CrossFadeState.showFirst : CrossFadeState.showSecond,
                                                    ...
                                                    Row(
                                                      children: [
                                                        Container(
                                                          ...
                                                          decoration: BoxDecoration(color: Color(0xFFEFF4FF), borderRadius: BorderRadius.circular(12.r)),
                                                          alignment: Alignment.center,
                                                          child: const Icon(Icons.settings_rounded, color: _primary),
                                                        ),
                                                        SizedBox(width: 10.w),
                                                        Expanded(child: Text('الإعدادات', style: GoogleFonts.tajawal(fontSize: 18.sp, fontWeight: FontWeight.w900, color: Color(0xFF0F172A)))),
                                                      ],
                                                    ),
                                                    SizedBox(height: 10.h),
                                                    Row(
                                                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                                      children: [
                                                        Text('ذكّرني قبل (بالأيام):', style: GoogleFonts.tajawal(fontWeight: FontWeight.w800)),
                                                        Container(
                                                          padding: EdgeInsets.symmetric(horizontal: 10.w, vertical: 4.h),
                                                          decoration: BoxDecoration(color: Color(0xFFEFF4FF), borderRadius: BorderRadius.circular(8.r)),
                                                          child: Text('1 يوم', style: GoogleFonts.tajawal(fontWeight: FontWeight.w900, color: _primary)),
                                                        ),
                                                      ],
                                                    ),
                                                    GestureDetector(
                                                      ...
                                                      onTapDown: (_) {
                                                        ScaffoldMessenger.of(sheetCtx).showSnackBar(SnackBar(content: Text('دورة السداد اليومية: التنبيه فقط قبل يوم واحد', style: GoogleFonts.tajawal(fontWeight: FontWeight.w800))));
                                                      },
                                                      ...
                                                      child: AbsorbPointer(
                                                        absorbing: true,
                                                        child: Slider.adaptive(value: 1.0, min: 1.0, max: 2.0, divisions: 1, onChanged: null, label: '1'),
                                                      ),
                                                    ),
                                                    SizedBox(height: 8.h),
                                                    Text('ملاحظة: دورة السداد اليومية — التنبيه فقط قبل يوم واحد ولا يمكن تعديله.', style: GoogleFonts.tajawal(fontWeight: FontWeight.w800, color: Color(0xFFB91C1C))),
                                                  ],
                                                ),
...
contractCard(
                                          ...
                                          onDaysChanged: (v) => setState(() => monthlyDays = v),
                                          expanded: monthlyOpen,
                                          onToggleExpanded: () => setState(() {
  monthlyOpen = !monthlyOpen;
  if (monthlyOpen) { dailyOpen = quarterlyOpen = semiOpen = annualOpen = false; }
}),

                                          mode: monthlyMode,
                                          onMode: (m) => setState(() => monthlyMode = m),
                                          beforeCtrl: msgMonthlyBeforeCtrl!,
                                          onCtrl: msgMonthlyOnCtrl!,
                                          saveFieldDays: 'notif_monthly_days',
                                          saveFieldMsgBefore: 'notif_monthly_msg_before',
                                          saveFieldMsgOn: 'notif_monthly_msg_on',
                                        ),
                                        contractCard(
                                          ...
                                          onDaysChanged: (v) => setState(() => quarterlyDays = v),
                                          expanded: quarterlyOpen,
                                          onToggleExpanded: () => setState(() {
  quarterlyOpen = !quarterlyOpen;
  if (quarterlyOpen) { dailyOpen = monthlyOpen = semiOpen = annualOpen = false; }
}),

                                          mode: quarterlyMode,
                                          onMode: (m) => setState(() => quarterlyMode = m),
                                          beforeCtrl: msgQuarterlyBeforeCtrl!,
                                          onCtrl: msgQuarterlyOnCtrl!,
                                          saveFieldDays: 'notif_quarterly_days',
                                          saveFieldMsgBefore: 'notif_quarterly_msg_before',
                                          saveFieldMsgOn: 'notif_quarterly_msg_on',
                                        ),
                                        contractCard(
                                          ...
                                          maxDays: 30,
                                          valueDays: semiDays,
                                          onDaysChanged: (v) => setState(() => semiDays = v),
                                          expanded: semiOpen,
                                          onToggleExpanded: () => setState(() {
  semiOpen = !semiOpen;
  if (semiOpen) { dailyOpen = monthlyOpen = quarterlyOpen = annualOpen = false; }
}),

                                          mode: semiMode,
                                          onMode: (m) => setState(() => semiMode = m),
                                          beforeCtrl: msgSemiBeforeCtrl!,
                                          onCtrl: msgSemiOnCtrl!,
                                          saveFieldDays: 'notif_semiannual_days',
                                          saveFieldMsgBefore: 'notif_semiannual_msg_before',
                                          saveFieldMsgOn: 'notif_semiannual_msg_on',
                                        ),
                                        contractCard(
                                          title: 'دفعات سنوية',
                                          minDays: 1,
                                          ...
                                          onDaysChanged: (v) => setState(() => annualDays = v),
                                          expanded: annualOpen,
                                          onToggleExpanded: () => setState(() {
  annualOpen = !annualOpen;
  if (annualOpen) { dailyOpen = monthlyOpen = quarterlyOpen = semiOpen = false; }
}),

                                          mode: annualMode,
                                          onMode: (m) => setState(() => annualMode = m),
                                          beforeCtrl: msgAnnualBeforeCtrl!,
                                          onCtrl: msgAnnualOnCtrl!,
                                          saveFieldDays: 'notif_annual_days',
                                          saveFieldMsgBefore: 'notif_annual_msg_before',
  ...
  if (cDailyOpen) { cMonthlyOpen = cQuarterlyOpen = cSemiOpen = cAnnualOpen = false; }
}),

                                                borderRadius: BorderRadius.circular(12.r),
                                                child: Row(
                                                  children: [
                                                    Expanded(child: Text('عقد يومي (يوم)', style: GoogleFonts.tajawal(fontWeight: FontWeight.w900, fontSize: 15.5.sp))),
                                                    const Icon(Icons.expand_more_rounded, color: _primary),
                                                  ],
                                                ),
                                              ),
                                              AnimatedCrossFade(
                                                crossFadeState: cDailyOpen ? CrossFadeState.showFirst : CrossFadeState.showSecond,
                                                duration: Duration(milliseconds: 180),
                                                firstChild: Column(
                                                  crossAxisAlignment: CrossAxisAlignment.start,
                                                  children: [
                                                    ...
                                                    GestureDetector(
                                                      behavior: HitTestBehavior.opaque,
                                                      onTapDown: (_) {
                                                        ScaffoldMessenger.of(sheetCtx).showSnackBar(SnackBar(content: Text('التنبيه فقط قبل يوم واحد من العقد اليومي ولا يمكن تغييره', style: GoogleFonts.tajawal(fontWeight: FontWeight.w800))));
                                                      },
                                                      onHorizontalDragStart: (_) {
                                                        ScaffoldMessenger.of(sheetCtx).showSnackBar(SnackBar(content: Text('التنبيه فقط قبل يوم واحد من العقد اليومي ولا يمكن تغييره', style: GoogleFonts.tajawal(fontWeight: FontWeight.w800))));
                                                      },
                                                    ...
                                                    SizedBox(height: 8.h),
                                                    Text('ملاحظة: العقد اليومي — التنبيه فقط قبل يوم واحد ولا يمكن تغييره.', style: GoogleFonts.tajawal(fontWeight: FontWeight.w800, color: Color(0xFFB91C1C))),
                                                  ],
                                                ),
                                                secondChild: SizedBox.shrink(),
                                              ),
...
contractCardSimple(
                                          ...
                                          onDaysChanged: (v) => setState(() => cMonthlyDays = v),
                                          expanded: cMonthlyOpen,
                                          onToggleExpanded: () => setState(() {
  cMonthlyOpen = !cMonthlyOpen;
  if (cMonthlyOpen) { cDailyOpen = cQuarterlyOpen = cSemiOpen = cAnnualOpen = false; }
}),

                                          beforeCtrl: cMsgMonthlyBeforeCtrl!,
                                          saveFieldDays: 'notif_contract_monthly_days',
                                          saveFieldMsgBefore: 'notif_contract_monthly_msg_before',
                                        ),
                                        contractCardSimple(
                                          ...
                                          onDaysChanged: (v) => setState(() => cQuarterlyDays = v),
                                          expanded: cQuarterlyOpen,
                                          onToggleExpanded: () => setState(() {
  cQuarterlyOpen = !cQuarterlyOpen;
  if (cQuarterlyOpen) { cDailyOpen = cMonthlyOpen = cSemiOpen = cAnnualOpen = false; }
}),

                                          beforeCtrl: cMsgQuarterlyBeforeCtrl!,
                                          saveFieldDays: 'notif_contract_quarterly_days',
                                          saveFieldMsgBefore: 'notif_contract_quarterly_msg_before',
                                        ),
                                        contractCardSimple(
                                          ...
                                          maxDays: 30,
                                          valueDays: cSemiDays,
                                          onDaysChanged: (v) => setState(() => cSemiDays = v),
                                          expanded: cSemiOpen,
                                          onToggleExpanded: () => setState(() {
  cSemiOpen = !cSemiOpen;
  if (cSemiOpen) { cDailyOpen = cMonthlyOpen = cQuarterlyOpen = cAnnualOpen = false; }
}),

                                          beforeCtrl: cMsgSemiBeforeCtrl!,
                                          saveFieldDays: 'notif_contract_semiannual_days',
                                          saveFieldMsgBefore: 'notif_contract_semiannual_msg_before',
                                        ),
                                        contractCardSimple(
                                          title: 'عقد سنوي (سنة)',
                                          minDays: 1,
                                          ...
                                          onDaysChanged: (v) => setState(() => cAnnualDays = v),
                                          expanded: cAnnualOpen,
                                          onToggleExpanded: () => setState(() {
  cAnnualOpen = !cAnnualOpen;
  if (cAnnualOpen) { cDailyOpen = cMonthlyOpen = cQuarterlyOpen = cSemiOpen = false; }
}),

                                          beforeCtrl: cMsgAnnualBeforeCtrl!,
                                          saveFieldDays: 'notif_contract_annual_days',
                                          saveFieldMsgBefore: 'notif_contract_annual_msg_before',
                                        ),
                                      ],
  ...
  onPressed: () async {
    ...
    if (await OfficeClientGuard.blockIfOfficeClient(sheetCtx)) return;

    await _confirmFactoryReset(sheetCtx);
  },
  icon: const Icon(Icons.delete_forever_rounded),
  label: Text('إعادة ضبط كامل', style: GoogleFonts.tajawal(fontWeight: FontWeight.w900)),
  style: ElevatedButton.styleFrom(
    backgroundColor: const Color(0xFFEF4444),
    foregroundColor: Colors.white,
  ...
  onPressed: () async {
    ...
    _saveSettings({
      ...
      'notif_monthly_msg_before': msgMonthlyBeforeCtrl!.text,
      'notif_monthly_msg_on': msgMonthlyOnCtrl!.text,
      'notif_quarterly_days': quarterlyDays,
      'notif_quarterly_msg_before': msgQuarterlyBeforeCtrl!.text,
      'notif_quarterly_msg_on': msgQuarterlyOnCtrl!.text,
      'notif_semiannual_days': semiDays,
      'notif_semiannual_msg_before': msgSemiBeforeCtrl!.text,
      'notif_semiannual_msg_on': msgSemiOnCtrl!.text,
      'notif_annual_days': annualDays,
      'notif_annual_msg_before': msgAnnualBeforeCtrl!.text,
      'notif_annual_msg_on': msgAnnualOnCtrl!.text,
      // العقود
      'notif_contract_monthly_days': cMonthlyDays,
      'notif_contract_monthly_msg_before': cMsgMonthlyBeforeCtrl!.text,
      'notif_contract_quarterly_days': cQuarterlyDays,
      'notif_contract_quarterly_msg_before': cMsgQuarterlyBeforeCtrl!.text,
      'notif_contract_semiannual_days': cSemiDays,
      'notif_contract_semiannual_msg_before': cMsgSemiBeforeCtrl!.text,
      'notif_contract_annual_days': cAnnualDays,
      'notif_contract_annual_msg_before': cMsgAnnualBeforeCtrl!.text,
    }, sheetCtx);
  },
  style: ElevatedButton.styleFrom(
  ...
  child: Text('حفظ', style: GoogleFonts.tajawal(fontWeight: FontWeight.w900, color: Colors.white)),
)
