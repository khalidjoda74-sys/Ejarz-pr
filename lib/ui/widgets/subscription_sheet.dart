...
Future<void> showSubscriptionSheet(
  ...
  DateTime toKsaDateOnly(DateTime dt) {
    final utc = dt.toUtc(); // نتأكد أنه UTC
    final ksa = utc.add(const Duration(hours: 3)); // نحوله لـ KSA
    return DateTime(
        ksa.year, ksa.month, ksa.day); // نحذف الساعة ونحتفظ باليوم فقط
  }

  // ✅ نثبت أن البداية والنهاية حسب تاريخ السعودية فقط
  final startDateKsa = toKsaDateOnly(startAt);
  final endDateKsa = toKsaDateOnly(endAt);

  // ✅ اليوم الحالي بتوقيت السعودية
  final nowKsa = KsaTime.now();
  final nowDateKsa = DateTime(nowKsa.year, nowKsa.month, nowKsa.day);
    ...
    builder: (_) => Directionality(
      ...
      child: Padding(
        padding: EdgeInsets.fromLTRB(
            16.w, 12.h, 16.w, 16.h + MediaQuery.of(context).viewInsets.bottom),
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                Container(
                  ...
                  child:
                      const Icon(Icons.card_membership_rounded, color: primary),
                ),
                SizedBox(width: 10.w),
                Expanded(
                  child: Text(
            ...
            if (active)
              _RowItem(label: 'الأيام المتبقية', value: '$leftDays يوم'),
