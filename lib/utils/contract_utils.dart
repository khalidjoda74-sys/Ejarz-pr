...
import '../ui/invoices_screen.dart' show Invoice, kInvoicesBox;
import '../data/constants/boxes.dart';
import 'package:hive/hive.dart';
import '../data/services/user_scope.dart';   // يوفر الدالة boxName
import '../data/constants/boxes.dart';       // يوفر الثابت kInvoicesBox
...
bool isDueToday(Contract c) {
  ...
  return _dateOnly(d) == _dateOnly(DateTime.now());
}
...
double perCycleAmountForContract(Contract c) {
  ...
  final int months = _monthsInTerm(c.term);
  final int cycles = (months / _monthsPerCycle(c.paymentCycle)).ceil().clamp(1, 1000);
  // If "deduct from total", reduce the total by advancePaid before dividing
  final double totalForCycles = (c.advanceMode.toString().contains('deductFromTotal'))
...
double sumReceivablesFromContracts({DateTime? from, DateTime? to, bool includeArchived = false}) {
  double total = 0.0;
  try {
    ...
    for (final c in box.values) {
      ...
      final int stepM = _monthsPerCycle(c.paymentCycle);
      ...
      while (cursor.isBefore(end) && guard < 600) {
        ...
        if (unpaid) {
          ...
          if (from == null && to == null) {
            ...
            include = true;
            if (from != null && cursor.isBefore(_dateOnly(from))) include = false;
...
double sumReceivablesFromContractsExact({DateTime? from, DateTime? to, bool includeArchived = false}) {
  int totalCents = 0;
  try {
    final String cBoxName = hs.HiveService.contractsBoxName();
    if (!Hive.isBoxOpen(cBoxName)) return 0.0;
    ...
    for (final c in box.values) {
      try {
        ...
        try { isArchived = (c as dynamic).isArchived == true; } catch (_) {}
        if (!includeArchived && isArchived) continue;
        ...
        final DateTime rangeTo   = (to != null) ? _dOnly(to) : today;
        final DateTime rangeFrom = (from != null) ? _dOnly(from) : DateTime(2000);

        // effective end is min(end, terminatedAt, rangeTo + 1 day), then end-exclusive iterate in loop.
        DateTime effEnd = DateTime(rangeTo.year, rangeTo.month, rangeTo.day + 1);
        if (end.isBefore(effEnd)) effEnd = end;
        if (isTerminated && terminatedAt != null && terminatedAt.isBefore(effEnd)) effEnd = terminatedAt;
        if (!start.isBefore(effEnd)) continue; // no room

        // Per-cycle amount in cents (truncate)
        int stepM = _monthsPerCycle((c as dynamic).paymentCycle as PaymentCycle);
        int cycles;
        if ((c as dynamic).term == ContractTerm.daily) {
          cycles = 1;
        } else {
          final int termMonths = _monthsInTerm((c as dynamic).term as ContractTerm);
        ...
        int perCycleCents = _toCentsTrunc((c as dynamic).term == ContractTerm.daily
            ? totalAmount
