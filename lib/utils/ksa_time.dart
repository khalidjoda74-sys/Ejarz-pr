...
class KsaTime {
  ...
  static Future<void> ensureSynced({bool force = false}) async {
    ...
    try {
      ...
      await _tsRef.set({'ts': FieldValue.serverTimestamp()}, SetOptions(merge: true));
      ...
      if (ts == null) {
        // في حالات نادرة جدًا قد يتأخر حلّ serverTimestamp،
        // نحافظ على fallback آمن (بدون وسم التهيئة).
      ...
      // عدم توفّر الإنترنت أو خطأ Firestore: نعمل بالوقت المحلي بدون وسم تهيئة.
      _drift = Duration.zero;
      _initialized = false;
    }
    ...
    try { HijriCalendar.setLocal('ar'); } catch (_) {}
  }
  ...
  static DateTime toKsa(DateTime any) => any.toUtc().add(const Duration(hours: 3));

  /// تحويل تاريخ/وقت KSA إلى UTC (للتخزين).
  static DateTime fromKsaToUtc(DateTime ksa) => ksa.toUtc().subtract(const Duration(hours: 3));
  ...
  static DateTime addMonthsKsaDateOnlyUtc(DateTime startKsaDateOnlyUtc, int months) {
  ...
  static DateTime endExclusiveUtcFromInclusiveDateOnlyUtc(DateTime endInclusiveDateOnlyUtc) =>
  ...
  static DateTime ksaDateTimeUtc(int y, int m, int d, {int hour = 0, int minute = 0}) {
    return DateTime.utc(y, m, d, hour, minute).subtract(const Duration(hours: 3));
  }
  ...
  static DateTime nightlyEndExclusiveUtcFromStart({
    ...
    final checkoutDateKsa = DateTime(sKsa.year, sKsa.month, sKsa.day).add(Duration(days: nights));
  ...
  /// صياغات شائعة
  static String formatKsaDate(DateTime any) => formatKsa(any, pattern: 'yyyy/MM/dd');
  static String formatKsaDateTime(DateTime any) => formatKsa(any, pattern: 'yyyy/MM/dd HH:mm');

  /// يعرض التاريخ حسب النظام المختار انطلاقًا من "لحظة" (UTC/محلي) بعد تحويلها إلى KSA.
  /// system: 'gregorian' | 'hijri'
  static String formatWithSystem(DateTime any, String system, {String gPattern = 'yyyy/MM/dd'}) {
  ...
  static String formatDateOnlyWithSystem(DateTime ksaDateOnly, String system, {String gPattern = 'yyyy/MM/dd'}) {
    if (system == 'hijri') {
