...
export function ContentPage() {
  ...
  return <div className="stack"><PageHeader title="إدارة محتوى التطبيق" subtitle="تحكم في النصوص والروابط ووضع الصيانة من appContent/config." actions={<><Button variant="soft" onClick={load}>تحديث</Button><Button variant="gold" onClick={save} disabled={saving}>{saving ? 'جاري الحفظ...' : 'حفظ التعديلات'}</Button></>} />
    <Card style={{ padding: 18 }}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}><div><h2 className="section-title">وضع الصيانة</h2><p className="page-subtitle">تفعيل هذا الخيار يجب أن يكون مدعومًا في التطبيق.</p></div><Button variant={form.maintenanceMode ? 'danger' : 'soft'} onClick={() => setConfirmMaintenance(true)}>{form.maintenanceMode ? 'الصيانة مفعلة' : 'الصيانة غير مفعلة'}</Button></div></Card>
    ...
    <ConfirmDialog open={confirmMaintenance} danger title="تغيير وضع الصيانة؟" description="سيتم حفظ قيمة maintenanceMode في النموذج، اضغط حفظ التعديلات بعدها لتثبيت التغيير." confirmLabel="تغيير" onCancel={() => setConfirmMaintenance(false)} onConfirm={() => { updateField('maintenanceMode', !form.maintenanceMode); setConfirmMaintenance(false); }} />
  </div>;
}
